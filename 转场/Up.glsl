...
vec4 transition (vec2 uv) {
  ...
  if (a<0.5) {
    if (uv.y < a*2.0){
      return linerBlur(from, (getMirrorBottom(AddMoveUp(uv, a*2.0))), (a)/0.5, 1);
    } else {
      return linerBlur(from, (AddMoveUp(uv,a*2.0)), (a)/0.5, 1);
    }
  } else {
    if (uv.y < (a-0.5)*2.0){
      return linerBlur(to, (AddMoveUp(uv,(a-1.)*2.0)), (1.0-a)/0.5, 1);
    } else {
      return linerBlur(to, getMirrorUp(AddMoveUp(uv, (a-1.)*2.0)), (1.0-a)/0.5, 1);
    }
  }
}