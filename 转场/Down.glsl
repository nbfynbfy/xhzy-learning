...
vec4 linerBlur(sampler2D texture, vec2 texCoord, float progress, int key) {
    ...
    vec2 direct;
    if (key == 0) { direct = vec2(0., 0.); }
    else if (key == 1) { direct = vec2(0., 1.); }
    else if (key == 2) { direct = vec2(0., -1.); }
    else if (key == 3) { direct = vec2(1., 0.); }
    else if (key == 4) { direct = vec2(-1., 0.); }
    else if (key == 5) { direct = vec2(1., 1.); }
...
vec4 transition (vec2 uv) {
  ...
  if (a<0.5) {
    if (uv.y > 1.-a*2.0){
      return linerBlur(from, (getMirrorUp(AddMoveDown(uv, a*2.0))), (a)/0.5, 1);
    } else {
      return linerBlur(from, (AddMoveDown(uv,a*2.0)), (a)/0.5, 1);
    }
  } else {
    if (uv.y > 1.-(a-0.5)*2.0){
      return linerBlur(to, (AddMoveDown(uv,(a-1.)*2.0)), (1.0-a)/0.5, 1);
    } else {
      return linerBlur(to, getMirrorBottom(AddMoveDown(uv, (a-1.)*2.0)), (1.0-a)/0.5, 1);
    }
  }
}